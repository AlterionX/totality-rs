use super::{
    hal::{
        ...
        memory::{Requirements, Properties},
...
impl <B: Backend> AllocatedBuffer<B> {
    pub fn new(
        adapter: &Adapter<B>, dev: &<B>::Device,
        name: Option<String>,
        sz: u64, usage: Usage
    ...
    pub fn load_data<T: Copy, F: FnOnce(&mut Writer<B, T>)>(
        ...
        unsafe {
            let mut data_target = device.acquire_mapping_writer(&self.mem, 0..self.reqs.size)
    ...
    pub fn load_data_from_slice(&self, device: &<B>::Device, sliced: &[u32], dst_offset: usize) -> Result<(), &'static str> {
        self.load_data(device, |target| {
            target[dst_offset..(dst_offset + sliced.len())]
...
impl <T, B: Backend> LoadedBuffer<T, B> {
    pub fn new(
        adapter: &Adapter<B>, dev: &mut <B>::Device,
        name: Option<String>,
        sz: u64, usage: Usage,
