...
impl <'link, T: Simulated, DL: DataLinkage<T>> DataLinkageGuard<'link, T, DL> {
    pub fn new(src: &'link DL) -> DataLinkageGuard<'link, T, DL> {
        DataLinkageGuard { src, phantom: std::marker::PhantomData }
    }
    pub fn source(&self) -> &'link T { self.src.source().expect("Created with a thing that was not unlocked.") }
    pub fn target(&self) -> &'link mut T { self.src.target().expect("Created with a thing that was not unlocked.") }
}
