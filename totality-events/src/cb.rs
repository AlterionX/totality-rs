...
pub trait CBFn<G: ValueStore<C, V>, V: Categorized<C>, C: Hash + Eq + PartialEq + Copy + Clone>:
    FnMut(&G, &V, &Instant, &Instant) + Send + 'static
{
}
impl<
        T: FnMut(&G, &V, &Instant, &Instant) + Send + 'static,
        G: ValueStore<C, V>,
        V: Categorized<C>,
        C: Hash + Eq + PartialEq + Copy + Clone,
    > CBFn<G, V, C> for T
{
}
...
impl<G: ValueStore<C, V>, V: Categorized<C>, C: Hash + Eq + PartialEq + Copy + Clone>
    Manager<G, V, C>
{
    pub fn new() -> Self {
        Manager {
    ...
    fn fire_category_events(
        &self,
        s: &G,
        c: &C,
        v: &V,
        curr_inst: &Instant,
    ) -> (C, Vec<Arc<Mutex<CB<G, V, C>>>>) {
    ...
    fn remove_matching(&mut self, removals: Vec<(C, Vec<Arc<Mutex<CB<G, V, C>>>>)>) {
        for (c, removal) in removals {
            let bucket = self.buckets.get_mut(&c).expect(
                "Bug in data structure maintenance. Requested removal of nonexistent callbacks.",
            );
            Self::remove_matching_arcs(bucket, removal);
            if bucket.len() == 0 {
                Self::remove_category(&mut self.occupied, &c);
            }
        }
    }
    pub fn register(&mut self, cb: CB<G, V, C>) -> Weak<Mutex<CB<G, V, C>>> {
        if !self.buckets.contains_key(&cb.c) {
            self.occupied.push(cb.c.clone())
        }
        let v = self
            .buckets
            .entry(cb.c.clone())
