...
pub fn run_test() {
    ...
    match create_kt!(
        ...
        {
            info!(
                "We're looping!! We've reached this point {:?} times.",
                count
            );
        ...
        {
            info!(
                "Welp, guess we're through here. Here's the vec, but the way: {:?}",
                hello
            );
        ...
        Ok(kt) => {
            ...
            match rx.try_recv() {
                Ok(v) => panic!(
                    "We received a value ({:?}) after it should've been dropped...",
                    v
                ),
                Err(std::sync::mpsc::TryRecvError::Empty) => {
                    panic!("The channel should've dropped by now!")
                }
                Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                    info!("Successfully disconnected from the thread.")
                }
            }
            match kt.finish() {
                Some(res) => match res {
                    Ok(s) => assert_eq!(
                        "A random string is your reward!", s,
                        "We should have gotten this string back..."
                    ),
                    Err(e) => panic!(
                        "Oh shit! We failed somewhere to join! Here's the error: {:?}",
                        e
                    ),
                },
